...
mkdir ${HOME}/.mande-chain && mkdir ${HOME}/.mande-chain/config
mande-chaind init $NODE_NAME --chain-id mande-testnet-1

wget https://raw.githubusercontent.com/mande-labs/testnet-1/main/genesis.json -c -O ${HOME}/.mande-chain/config/genesis.json
...
sed -i 's|^seeds *=.*|seeds = "'$SEEDS'"|; s|^persistent_peers *=.*|persistent_peers = "'$PEERS'"|' $HOME/.mande-chain/config/config.toml
